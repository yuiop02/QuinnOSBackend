...
app.post('/run-stream', async (req, res) => {
  ...
  try {
    runStage = 'parse_request';
    packet = String(req.body?.packet || '').trim();
    prompt = String(req.body?.prompt || '').trim();
    projectTag = normalizeProjectTag(req.body?.projectTag);
    threadId = cleanMemoryText(req.body?.threadId || '');
    ...
    memorySections = buildRunMemorySections(memory, {
      packet,
      projectTag,
      previousAssistantReply: cleanMemoryText(req.body?.previousAssistantReply || ''),
      threadId,
    });
    ...
    const trimmedPrompt = String(
      prompt ||
        'Reply as Ren speaking directly to Quinn. Be direct, warm, emotionally accurate, specific, and conversational. Use structure only if the user clearly asks for it.'
    ).slice(0, 500);
    ...
    const instructions = [
      REN_CORE_V1,
      'You are Ren in QuinnOS: a separate conversational mirror speaking back to Quinn.',
      'Use the current packet as the live thing being said right now.',
      'Answer the actual packet first.',
      'Use already-known terrain quietly if it genuinely improves relevance.',
      'Do not narrate memory, continuity, or internal mechanics.',
      'Do not sound like a tool, coach, therapist, analyst, or memo.',
      'Default to prose-first. Do not use bullets unless Quinn asks for structure.',
      'Do not offer follow-up questions or options unless they are genuinely needed.',
      'Do not use "if you want" endings.',
      'Keep the reply vivid, direct, and natural.',
      'Stop where the point lands.',
    ].join(' ');
...
- Keep the answer plain, narrow, and obedient.`,
    ...
    runTimer.mark('input_built');

    runStage = 'provider_stream_request';
    ...
    for await (const event of stream) {
      if (event.type === 'response.output_text.delta') {
        ...
        if (delta) {
          ...
          sendEvent('delta', {
            text: delta,
            elapsedMs: Date.now() - runTimer.finish().totalMs + runTimer.finish().totalMs,
          });
        }
      }
      ...
      if (event.type === 'response.completed') {
        ...
        runTimer.mark('provider_completed');
        break;
      }

      if (event.type === 'response.failed') {
        throw new Error(event.response?.error?.message || 'Streaming response failed');
      }
    ...
    if (memory) {
      if (responseId) {
        memory.lastResponseId = responseId;
      }
    ...
    runStage = 'respond_done';
    runTimer.mark('respond_done');
    ...
    sendEvent('done', {
      ...
      timings: runTimer.finish({
        runStage,
        firstDeltaMs: firstDeltaAt ? firstDeltaAt - (Date.now() - runTimer.finish().totalMs) : null,
        providerMs: providerStartedAt ? Date.now() - providerStartedAt : null,
      }),
    });

    res.end();
  } catch (error) {
    console.error('RUN STREAM ERROR:', {
      stage: runStage,
      message: error?.message || 'Run stream failed',
      name: error?.name || 'Error',
    });
    ...
    if (!res.headersSent) {
      return res.status(500).json({
        ok: false,
        error: error?.message || 'Run stream failed',
      });
    }
